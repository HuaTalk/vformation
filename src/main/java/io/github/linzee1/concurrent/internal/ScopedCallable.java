...
public class ScopedCallable<V> implements Callable<V>, Attachable {
    ...
    private void notifyListeners(Throwable exception) {
        List<TaskListener> listeners = StructuredParallel.getTaskListeners();
        ...
        for (TaskListener listener : listeners) {
            try {
                ...
                StructuredParallel.getLogger().warn("TaskListener callback failed: {}", listener.getClass().getName(), e);
            }
        }
    }
