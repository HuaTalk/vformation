...
public class PurgeService {
    ...
    public static ListenableFuture<?> tryPurge(String executorName, Map.Entry<Map<State, Integer>, Throwable> report) {
        ...
        return getPurgeExecutor().submit(() -> {
            ThreadPoolExecutor executor = StructuredParallel.resolveThreadPool(executorName);
            if (executor == null) {
                StructuredParallel.getLogger().debug("Cannot resolve thread pool '{}' for purge", executorName);
                return false;
            }
