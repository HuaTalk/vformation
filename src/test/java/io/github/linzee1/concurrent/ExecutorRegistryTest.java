...
public class ExecutorRegistryTest {
    ...
    public void tearDown() {
        ...
        StructuredParallel.unregisterExecutor(POOL_NAME);
        StructuredParallel.setExecutorResolver(null);
    ...
    public void testRegisterGetUnregisterLifecycle() {
        assertNull(StructuredParallel.getExecutor(POOL_NAME));

        StructuredParallel.registerExecutor(POOL_NAME, executor);
        ListeningExecutorService retrieved = StructuredParallel.getExecutor(POOL_NAME);
    ...
    public void testRegisterWithNullNameThrows() {
        assertThrows(IllegalArgumentException.class,
                () -> StructuredParallel.registerExecutor(null, executor));
    }

    @Test
    public void testRegisterWithEmptyNameThrows() {
        assertThrows(IllegalArgumentException.class,
                () -> StructuredParallel.registerExecutor("", executor));
    }

    @Test
    public void testRegisterWithNullExecutorThrows() {
        assertThrows(IllegalArgumentException.class,
                () -> StructuredParallel.registerExecutor(POOL_NAME, null));
    }
    ...
    public void testResolveThreadPoolFromRegistry() {
        ...
        try {
            StructuredParallel.registerExecutor(POOL_NAME, tpe);

            // No explicit ExecutorResolver set
            ThreadPoolExecutor resolved = StructuredParallel.resolveThreadPool(POOL_NAME);
    ...
    public void testExplicitExecutorResolverTakesPriority() {
        ...
        try {
            StructuredParallel.registerExecutor(POOL_NAME, registryTpe);
